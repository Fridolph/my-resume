...
const itemBadgeClass = 'min-h-8 rounded-full px-[0.7rem] py-[0.42rem] text-[0.72rem] font-bold tracking-[0.04em]';
const itemTagClass =
    ...
    >
      <div className="grid gap-4">
        {projects.map((project) => {
          ...
          return (
            <article className={timelineItemClass} key={`${project.name.en}-${project.startDate}`}>
              ...
              {summary ? (
                <div className={itemBlockClass}>
                  <p className={itemBlockLabelClass}>{locale === 'zh' ? '项目概览' : 'Summary'}</p>
              ...
              {project.technologies.length > 0 ? (
                <div className={itemBlockClass}>
                  <p className={itemBlockLabelClass}>{locale === 'zh' ? '技术栈' : 'Tech Stack'}</p>
                  <TagGroup.Root aria-label={`${projectName} 技术栈`}>
                    ...
                    >
                      {(tech) => (
                        <Tag.Root className={itemTagClass} id={tech.id} textValue={tech.label}>
                          {tech.label}
                        </Tag.Root>
                      )}
                    </TagGroup.List>
              ...
              {project.links.length > 0 ? (
                <div className={itemBlockClass}>
                  <p className={itemBlockLabelClass}>{locale === 'zh' ? '项目链接' : 'Links'}</p>
